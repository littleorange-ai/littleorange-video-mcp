...
<p align="center">
  <img src="https://camo.githubusercontent." alt="Image" width="200"  />
...
| `LITTLEORANGE_CATALOG_AUTO_UPDATE`      | 接口目录自动更新开关（0关闭/1开启）          | 1                           | ❌ 否 |
| `LITTLEORANGE_CATALOG_REFRESH_SECONDS`  | 接口目录缓存刷新间隔（秒）；设为0表示每次都检查官方文档 | 3600                        | ❌ 否 |
| `LITTLEORANGE_CATALOG_FORCE_REFRESH`    | 强制刷新接口目录（1=每次都从Apifox拉取）     | 0                           | ❌ 否 |
| `LITTLEORANGE_CATALOG_LOG_FILE`         | 目录刷新日志本地存储路径                 | 无                           | ❌ 否 |
| `LITTLEORANGE_CATALOG_CACHE_FILE`       | 接口目录缓存文件路径                   | 系统缓存目录                      | ❌ 否 |
...
1. 临时设置环境变量：`LITTLEORANGE_CATALOG_FORCE_REFRESH=1`
2. 重启 MCP 服务
3. 获取更新后记得改回 `0` 避免频繁请求

#### Q8：目录刷新机制的工作原理是什么？

1. 默认每 1 小时检查一次 Apifox 文档更新
2. 如果缓存过期，从 Apifox 下载最新文档
3. 如果网络不可用，使用最近一次缓存
4. 没有缓存时使用包内置的兜底目录
...
- `LITTLEORANGE_DEBUG=1`
- `LITTLEORANGE_CATALOG_LOG_FILE=/path/to/catalog.log`
...
- `LITTLEORANGE_CATALOG_REFRESH_SECONDS=3600`：缓存刷新间隔，默认 1 小时；设为 `0` 表示每次启动都检查官方文档。
- `LITTLEORANGE_CATALOG_FORCE_REFRESH=0`：强制刷新接口目录，设为 `1` 表示每次都从 Apifox 拉取最新文档。
- `LITTLEORANGE_CATALOG_LOG_FILE=/path/to/catalog.log`：目录刷新日志文件路径（可选）。
- `LITTLEORANGE_CATALOG_CACHE_FILE=/path/to/api_catalog.json`：自定义缓存文件路径（可选）。

**快速获取最新接口方法**：

1. 设置环境变量：`LITTLEORANGE_CATALOG_FORCE_REFRESH=1`
2. 重启 MCP 服务
3. 获取更新后记得改回 `0` 避免频繁请求
